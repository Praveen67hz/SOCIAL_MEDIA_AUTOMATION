     ...
     if(!scheduledDate || !scheduledTime){
      ...
      try {
        await api.post("/api/posts",  {
          ...
          mediaType: activeScheduler.mediType,
    ...
    <div className="max-w-4xl mx-auto space-y-12 pb-20 animate-in fade-in duration-700">
      ...
      <div className="space-y-6 text-center mt-20">
        ...
        <div className="relative group mt-12">
            ...
            text-slate-900 placeholder-slate-400 outline-none focus:border-indigo-300 
            transition resize-none h-40"
            placeholder="Share your idea...(e.g. A post about the launch of our new
            eco-friendly coffee beans)" value={prompt} onChange={(e)=>setPrompt(e.target.value)}/>
            <div className="absolute bottom-4 right-2.5 flex items-center gap-3 text-sm">

               <button onClick={()=>setGenerateImage(!generateImage)} className="flex items-center gap-3
               bg-indigo-50 py-2 px-3 rounded-lg" >
                  <span>AI Image</span>
                  ...
                  ${generateImage ? "bg-indigo-500" : "bg-slate-200"}`}>
                   <span className={`pointer-events-none size-4 transform translate-y-0.5
                   rounded-full bg-white transition ${generateImage ? "translate-x-4.5" :
                    "translate-x-0.5"}`}/>
                  </div>
               </button>

               <button onClick = {handleGenerate} disabled = {loading} className="bg-indigo-500 hover:bg-indigo-600 text-white
         ...
         <div className="flex flex-wrap justify-center gap-2">
               {tones.map((t)=>(
                ...
                className={`px-4 py-1.5 rounded-full text-sm transition-all border ${tone === t ? "bg-indigo-500 border-indigo-500 text-white" : 
        ...
        <div className="space-y-6 pt-12 border-t border-slate-100">
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
               {generations.map((gen)=>(
                  ...
                  border-slate-100 p-5 hover:border-indigo-200 transition-all relative
                  overflow-hidden">
                    <div className="flex flex-col h-full space-y-4">
                       <div className="flex items-center justify-between">
                        <span className="text-xs text-slate-400
                        uppercase tracking-widest">{new Date(gen.createdAt).toLocaleString()}</span>
                        <span className="text-xs text-indigo-600 bg-indigo-50 px-2 py-0.5 
                       ...
                       {gen.mediaUrl && (
                        ...
                        bg-slate-50">
                          ...
                          object-cover opacity-90 group-hover:opacity-100" />
                        </div>
                       )}
                       <div className="flex items-center gap-2 pt-2">
                        <button 
                         onClick={()=> setActiveScheduler(gen)}
                         className="flex-1 bg-slate-100 hover:bg-indigo-500 hover:text-white
...

               {
                generations.length === 0 && (
                  <div className="col-span-full py-20 text-center space-y-2 ">
                    ...
                    <p>No content generated yet. 
                     Try generating some content using the AI.</p>
                  </div>
                )
               }
           </div>
        ...
        backdrop-blur-md animate-in fade-in duration-300">
          ...
          border border-slate-100 overflow-hidden flex flex-col max-h-[90vh]">
            ...
            border-b border-slate-100 bg-slate-50/30">
              ...
              text-slate-400 transition-colors"><XIcon className="size-5"/>
              </button>
            </div>
            <div className="flex-1 overflow-y-auto p-8 space-y-4">
                <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100 space-y-4">
                  <p className="text-slate-800 text-sm leading-relaxed 
                  whitespace-pre-wrap">{activeScheduler.prompt}</p>
                </div>

                  <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100 space-y-4">
                  <p className="text-slate-800 text-sm leading-relaxed 
                  whitespace-pre-wrap">{activeScheduler.content}</p>
                  {activeScheduler.mediaUrl && <img src={activeScheduler.mediaUrl}
                  alt="preview" className="w-full aspect-video object-cover rounded-xl
             ...
             <div className="p-8 bg-slate-50/50 border-t border-slate-50 space-y-8">
               ...
               <div className="space-y-6">
                  <div>
                    ...
                    mb-4">Select Channels</label>
                    <div className="flex flex-wrap gap-2">
                      {PLATFORMS.map((p)=>{
                        const active = selectedPlatforms.includes(p.id);
                        return(
                          <button key = {p.id} onClick={()=>setSelectedPlatforms((prev)=>(prev
                            .includes(p.id) ? prev.filter((x)=>x !== p.id) : [...prev,p.id]
                          ))} className={`p-2.5 rounded-md border text-xs ${active ? "bg-indigo-500/80 text-white" :
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className="relative">
                        ...
                        <input
                          type="date"
                          className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-100 
                          rounded-md text-slate-900 text-sm focus:outline-none focus:border-indigo-300 transition-all"
                          value={scheduledDate}
               ...
               <button onClick={handleSchedule} className="w-full  flex items-center justify-center gap-2
               py-3 rounded-md bg-slate-200 text-slate-700 hover:bg-indigo-500 hover:text-white
               transition">
