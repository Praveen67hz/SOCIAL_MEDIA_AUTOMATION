                ...
                return(
                     <button
                        ...
                        className={`flex items-center gap-3 p-3.5 rounded-xl border text-left transition-all ${
                            ...
                            : "border-slate-200 bg-slate-50 hover:border-indigo-300 hover:bg-indigo-50/50 cursor-pointer"
                        }${isConnecting && "opacity-60"}`} >

                        {/* Icon */}
                        <div className="p-2">
                            <p.icon className= {`size-5 ${isConnected ? "text-indigo-600":
                                "text-slate-500"}`}/>
                        </div>

                        {/* Label */}
                        <div className="flex-1 min-w-0">
                            <div className= {`text-sm ${isConnected ? "text-indigo-700":
                                "text-slate-800"}`}>
                                {p.name}
                            </div>
                        ...
                        {/* Status */}
                         {isConnected && <CheckCircleIcon className="size-4 text-indigo-500
                         shrink-0"/>}
                         {isConnecting && <div className="size-4 border-2 border-indigo-600
